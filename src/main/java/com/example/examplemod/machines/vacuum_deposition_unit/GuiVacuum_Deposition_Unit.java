...
public class GuiVacuum_Deposition_Unit extends GuiContainer {
    private final TileEntityVacuumDepositionUnit tileEntity;

    public GuiVacuum_Deposition_Unit(InventoryPlayer playerInv, TileEntityVacuumDepositionUnit tileEntity) {
        super(new ContainerVacuumDepositionUnit(playerInv, tileEntity));
...
//        this.fontRenderer.drawString("Silicon", 30, 7, 0xBABABA);
//        this.fontRenderer.drawString("Alum", 65, 7, 0xBABABA);
//        this.fontRenderer.drawString("Coal Block", 35, 45, 0xBABABA);
//        this.fontRenderer.drawString("Output", 120, 24, 0xBABABA);
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        drawSlotBack(i + 35, j + 18);  // Слот 0: Кремний
        drawSlotBack(i + 65, j + 18);  // Слот 1: Алюминий
        drawSlotBack(i + 125, j + 35); // Слот 2: Выход
        drawSlotBack(i + 50, j + 55);  // Слот 3: Угольный блок (топливо)
        ...
        drawRect(i + 12, j + 20, i + 20, j + 65, 0xFF121212); // Фон шкалы
        if (energy > 0) {
            drawRect(i + 13, j + 64 - energyBarHeight, i + 19, j + 64, 0xFF0055FF); // Синяя полоска
        }
    ...
    private void drawSlotBack(int x, int y) {
        drawRect(x - 1, y - 1, x + 17, y + 17, 0xFF121212); // Внешняя рамка
        drawRect(x, y, x + 16, y + 16, 0xFF8B8B8B);        // Фон слота
    }
